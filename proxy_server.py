...
__homepage__ = 'https://github.com/abhinavsingh/proxy_getter.py'
__license__ = 'BSD'
...
def pick_proxy():
    ...
    ip_str = redis_ins.zrange(REDIS_KEY, 0, 0)[0]
    redis_ins.zincrby(REDIS_KEY, ip_str)
    logger.debug("use proxy: " + ip_str)
    ip = ip_str.split(":")
    to_address = (ip[0], int(ip[1]))
    return to_address
...
class Server(Connection):
    ...
    def connect(self):
        ...
        self.conn.settimeout(CHECK_TIMEOUT)
        try:
            self.conn.connect(pick_proxy())
        except Exception as error:
            # reconnect use local ip
        ...
        if self.client.conn in r:
            ...
            try:
                ...
                self.client.queue(CRLF.join([
                    ...
                    b'Proxy-agent: proxy_getter.py v' + version,
                    b'Content-Length: 11',
                    b'Connection: close',
                    CRLF
                ]) + b'Bad Gateway')
...
class TCP(object):
    ...
    def __init__(self, hostname='127.0.0.1', port=8899, backlog=1000):
